...
int main(int argc, const char* argv[])
{
	...
	Console::get().writeLine(L"Connecting to nameserver as user " + gUserID);
	Console::get().writeLine();

	chatClient.connect(Chat::AuthenticationValues().setUserID(gUserID));
	while(true)
	{
		chatClient.service();
		Console::get().update();
